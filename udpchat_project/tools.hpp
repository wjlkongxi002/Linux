...
class TimeStamp
{
  public:
    ...
    static void GetTimeStamp(std::string* timestamp)
    {
       ...
       * */
      time_t sys_time;
      time(&sys_time);// 到目前的秒数

      struct tm* t = localtime(&sys_time);// 将秒数转换为年月日时分秒
      // yyyy-mm-dd HH:MM:SS
      char buf[30] = {'\0'};
...
std::ostream& Log(LogLevel loglovel, const char* file, int line, const std::string msg)
{
  std::string timestamp;
  TimeStamp::GetTimeStamp(&timestamp);

  std::string level = LevelInfo[loglovel];
